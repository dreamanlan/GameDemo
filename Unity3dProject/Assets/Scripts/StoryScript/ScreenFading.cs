...
public class ScreenFading : MonoBehaviour
{
    ...
    protected Material material
    {
        get
        {
            if (m_Material == null && MatShader != null) {
	...
	void ReleaseMaterial()
	{
		if (m_Material != null)
		{
			Utility.DestroyObjectFull(m_Material);
			m_Material = null;
		}
	}
    ...
    // message
    IEnumerator Fading(object[] args)
    {
        if (null != args && args.Length == 2) {
            long time1 = (long)System.Convert.ChangeType(args[0], typeof(long));
            long time2 = (long)System.Convert.ChangeType(args[1], typeof(long));
            DimScreen(time1);
            yield return new WaitForSeconds(time1 / 1000.0f);
            LightScreen(time2);
        }
        yield break;
    }
