...
namespace DslExpression
{
    ...
    internal sealed class DistExp : AbstractExpression
    {
        protected override object DoCalc()
        {
            ...
            object v = Geometry.Distance(new Vector2(x1, y1), new Vector2(x2, y2));
            return v;
        }
        protected override bool Load(IList<IExpression> exps)
        {
    ...
    internal sealed class DistSqrExp : AbstractExpression
    {
        protected override object DoCalc()
        {
            ...
            float x2 = (float)ToDouble(m_Op3.Calc());
            float y2 = (float)ToDouble(m_Op4.Calc());
            object v = Geometry.DistanceSquare(new Vector2(x1, y1), new Vector2(x2, y2));
            return v;
        }
        protected override bool Load(IList<IExpression> exps)
        {
            ...
            if (operands.Count >= 1) {
                ...
                if (operands.Count >= 2) {
                    ...
                    if (!string.IsNullOrEmpty(name)) {
                        ...
                        if (null != val)
                            valStr = val.ToString();
                        GameLibrary.GmCommands.GlobalDebugValues.SetValue(name, valStr);
                        ret = valStr;
                    }
                }
                else {
                    ret = GameLibrary.GmCommands.GlobalDebugValues.GetStringValue(name);
                }
            }
            return ret;
        }
